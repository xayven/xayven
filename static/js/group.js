...
const GROUP_STATE_KEY = 'xayven-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('xayven-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('xayven-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
