...
let _sortMode = Storage.get('helix-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'H E L I X Chat';
...
const FOLDER_STATE_KEY = 'helix-folder-state';
const FOLDER_ORDER_KEY = 'helix-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to H E L I X kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'H E L I X Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('helix-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('helix-session-sort', mode);
  else Storage.remove('helix-session-sort');
  renderSessionList();
}
