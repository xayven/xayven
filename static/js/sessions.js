...
let _sortMode = Storage.get('xayven-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Xayven Chat';
...
const FOLDER_STATE_KEY = 'xayven-folder-state';
const FOLDER_ORDER_KEY = 'xayven-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Xayven kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Xayven Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('xayven-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('xayven-session-sort', mode);
  else Storage.remove('xayven-session-sort');
  renderSessionList();
}
