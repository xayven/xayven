...
import * as Modals from './modalManager.js';
  ...
  async function _resolveComposeSendAccountId() {
    const activeAccountId = window.__xayvenActiveEmailAccount || null;
  ...
  const _docOpenKey = (sessionId) => 'xayven-doc-open-' + sessionId;
  const _docMinimizedKey = (sessionId) => 'xayven-doc-minimized-' + sessionId;
    ...
    try {
      const res = await fetch(`${API_BASE}/api/email/draft`, {
        ...
        body: JSON.stringify({
          ...
          account_id: window.__xayvenActiveEmailAccount || null,
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  export function openPanel() {
    ...
    let _fontIdx = parseInt(localStorage.getItem('xayven-doc-fontsize') || '0', 10);
    ...
    function _applyDocFont() {
      ...
      localStorage.setItem('xayven-doc-fontsize', _fontIdx);
    }
    _applyDocFont();
  ...
  const _DOC_RECENTS_KEY = 'xayven-doc-actions-recent';
  ...
  function _saveSuggestionsToStorage() {
    ...
    if (data.length) {
      localStorage.setItem('xayven-suggestions-' + activeDocId, JSON.stringify(data));
    } else {
      localStorage.removeItem('xayven-suggestions-' + activeDocId);
    }
  }
  ...
  function _restoreSuggestionsFromStorage(docId) {
    try {
      const raw = localStorage.getItem('xayven-suggestions-' + docId);
      if (!raw) return;
