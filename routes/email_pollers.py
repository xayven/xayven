                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Xayven (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Xayven" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "xayven_kind" if "xayven_kind" in cols else "'scheduled' AS xayven_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                xayven_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Xayven-Origin"] = "xayven-ui"
                outer["X-Xayven-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", xayven_kind or "scheduled")[:64]
                outer["X-Xayven-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `xayven-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `XAYVEN_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `xayven-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("XAYVEN_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `XAYVEN_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (XAYVEN_INPROCESS_POLLERS=0); "
            "drive `xayven-mail poll-scheduled` externally."
        )
        return
    import asyncio
