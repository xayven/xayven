                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in H E L I X (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in H E L I X" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "helix_kind" if "helix_kind" in cols else "'scheduled' AS helix_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                helix_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-H E L I X-Origin"] = "helix-ui"
                outer["X-H E L I X-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", helix_kind or "scheduled")[:64]
                outer["X-H E L I X-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `helix-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `HELIX_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `helix-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("HELIX_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `HELIX_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (HELIX_INPROCESS_POLLERS=0); "
            "drive `helix-mail poll-scheduled` externally."
        )
        return
    import asyncio
