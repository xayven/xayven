...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt Xayven' canonical OpenAI-style messages to native Ollama /api/chat.

    Xayven carries assistant tool calls in the OpenAI shape, where
...
def _detect_provider(url: str) -> str:
    ...
    if _host_match(url, "nvidia.com"):
        return "nvidia"
    if _host_match(url, "generativelanguage.googleapis.com"):
        return "google"
...
def _apply_local_cache_affinity(payload: Dict, url: str, session_id: Optional[str]) -> None:
    ...
    Sending a stable ``session_id`` (derived from the Xayven session) lets
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/samayran-ik/xayven-v3")
        h.setdefault("X-OpenRouter-Title", "Xayven")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip Xayven-only metadata before sending messages to providers.
